...
function extractToolInfo(filePath) {
  ...
  // 提取 canonical URL
  const canonicalMatch = content.match(/canonical:\s*['"]https:\/\/toolcase\.cc\/zh-tw\/([^'"]+)['"]/);
  ...
  return { slug, toolName, description };
}
...
// 生成 Schema 元件的 JSX
function generateSchemaJsx(toolName, description, slug) {
  const homeUrl = 'https://toolcase.cc';
  const toolUrl = `https://toolcase.cc/zh-tw/${slug}`;
...
function generateBreadcrumbsJsx(toolName) {
  return `        <Breadcrumbs
          items={[
            { name: '首頁', href: '/zh-tw' },
            { name: '${toolName}' },
...
function processFile(filePath, dryRun = false) {
  ...
  // 提取工具資訊
  const { slug, toolName, description } = extractToolInfo(filePath);
  ...
  // 生成 Schema JSX
  const schemaJsx = generateSchemaJsx(toolName, description, slug);
  const breadcrumbsJsx = generateBreadcrumbsJsx(toolName);
...
function main() {
  ...
  // 取得所有中文工具頁面
  const zhtwPagesDir = path.join(__dirname, '../src/app/zh-tw');
  const zhtwDirs = fs.readdirSync(zhtwPagesDir, { withFileTypes: true })
    .filter(dirent => dirent.isDirectory())
    .map(dirent => path.join(zhtwPagesDir, dirent.name, 'page.tsx'))
    .filter(p => fs.existsSync(p));

  let filesToProcess = zhtwDirs;

  if (testCount > 0) {
    console.log(`🧪 測試模式 - 只處理前 ${testCount} 個頁面\n`);
    filesToProcess = zhtwDirs.slice(0, testCount);
  }

  console.log(`📦 總共找到 ${zhtwDirs.length} 個中文頁面`);
